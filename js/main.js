...
function create_canvas() {
  ...
  if(canvas) {
    ...
    document.getElementById("canvas-container") ? document.getElementById("canvas-container").style.display = "block" : undefined;
  }
}
...
function undo() {
  ...
  if(history.length > 0 && hist_nav_idx <= history.length && hist_nav_idx > 0) {
...
function redo() {
  if(history.length > 0 && hist_nav_idx < history.length - 1 && hist_nav_idx >= -1) {
    var cvs = document.getElementById("canvas");
    var context = cvs.getContext("2d");
    var undo_img = new Image();
    undo_img.addEventListener("load", function() {
...
window.onload = function() {
  ...
  // let temp_rect_box = document.getElementById("temp_rect_box");
  ...
  canvas.addEventListener("mouseup", function(e) {
    if(e.button === 0) {
      ...
      if(brush === "pen") {
        ...
        ctx.rect(startX, startY, e.offsetX + 1 - startX, e.offsetY + 1 - startY);
      } else if (brush === "line") {
        ctx.lineTo(e.offsetX + 1, e.offsetY + 1);
      } else {
